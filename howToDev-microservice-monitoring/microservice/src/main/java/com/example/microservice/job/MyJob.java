...
public class MyJob {
  ...
  private final MeterProvider<Counter> myCounterProducer;
  ...
  public MyJob(MeterRegistry collectorRegistry) {
    ...
    myCounterProducer = Counter
        .builder("my_job_counter_produced_11")
        .tag("pod", podName)
        .description("myJob Counter Produced")
        .withRegistry(collectorRegistry);

    gauge = collectorRegistry.gauge("my_job_gouge_produced_12",
        new AtomicInteger(Long.valueOf(getRandomValue()).intValue()));
